...
public class EquipoLogic implements IEquipoLogic {
    ...
    */
    ...
    public void validateEquipo(Equipo equipo) throws Exception {
        try {
            java.util.Set<ConstraintViolation<Equipo>> constraintViolations = validator.validate(equipo);

            if (constraintViolations.size() > 0) {
                StringBuilder strMessage = new StringBuilder();

                for (ConstraintViolation<Equipo> constraintViolation : constraintViolations) {
    ...
    public void deleteEquipo(Equipo entity) throws Exception {
        ...
        List<JugadorEquipo> jugadorEquipos = null;
        List<Partido> partidosForEquipo1Id = null;
        List<Partido> partidosForEquipo2Id = null;
        List<Partido> partidosForEquipoGanadorId = null;
        List<Set> setsForEquipo1Id = null;
        List<Set> setsForEquipo2Id = null;

        try {
            equipoTorneos = equipoTorneoDAO.findByProperty("equipo.equipoId",
                    entity.getEquipoId());
            ...
            jugadorEquipos = jugadorEquipoDAO.findByProperty("equipo.equipoId",
                    entity.getEquipoId());

            if (Utilities.validationsList(jugadorEquipos) == true) {
                throw new ZMessManager().new DeletingException("jugadorEquipos");
            }

            partidosForEquipo1Id = partidoDAO.findByProperty("equipo.equipoId",
                    entity.getEquipoId());

            if (Utilities.validationsList(partidosForEquipo1Id) == true) {
                throw new ZMessManager().new DeletingException(
                    "partidosForEquipo1Id");
            }

            partidosForEquipo2Id = partidoDAO.findByProperty("equipo.equipoId",
                    entity.getEquipoId());

            if (Utilities.validationsList(partidosForEquipo2Id) == true) {
                throw new ZMessManager().new DeletingException(
                    "partidosForEquipo2Id");
            }

            partidosForEquipoGanadorId = partidoDAO.findByProperty("equipo.equipoId",
                    entity.getEquipoId());

            if (Utilities.validationsList(partidosForEquipoGanadorId) == true) {
                throw new ZMessManager().new DeletingException(
                    "partidosForEquipoGanadorId");
            }

            setsForEquipo1Id = setDAO.findByProperty("equipo.equipoId",
                    entity.getEquipoId());

            if (Utilities.validationsList(setsForEquipo1Id) == true) {
                throw new ZMessManager().new DeletingException(
                    "setsForEquipo1Id");
            }

            setsForEquipo2Id = setDAO.findByProperty("equipo.equipoId",
                    entity.getEquipoId());

            if (Utilities.validationsList(setsForEquipo2Id) == true) {
                throw new ZMessManager().new DeletingException(
                    "setsForEquipo2Id");
            }

            equipoDAO.deleteById(entity.getEquipoId());
    ...
    public List<EquipoDTO> getDataEquipo() throws Exception {
        try {
            ...
            for (Equipo equipoTmp : equipo) {
                EquipoDTO equipoDTO2 = equipoMapper.equipoToEquipoDTO(equipoTmp);
